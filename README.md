...
- Supply tracking emits events and updates unswapped totals automatically when funding is executed or exchanged.
- Revoking `CEO_ROLE` requires a ready successor and disallows self-removal; all CEO transitions emit `CeoChanged` events.
- ExchangeManager now exposes `setPriceFeed`, `increaseTotalUnswapped` and `decreaseTotalUnswapped` to keep supply and price logic modular.
- `setPriceFeed` seeds `lastPrice` to avoid stale comparisons and emits `PriceFeedUpdated`.
- `decreaseTotalVestedTokens` and `decreaseTotalUnswapped` revert if the amount exceeds current totals.
- Increasing `totalVestedTokens` or `totalUnswapped` fails if the new totals would exceed the overall token supply.
- Voting uses `ERC20Votes` snapshots taken one block prior, requiring voters to retain that balance until `tokenUnlockTime`. Only the excess is transferable and delegation is blocked while locked.
...
- `constructor.env` containing `_priceFeedAddress`, `_initialCeo`, `_initialEndorsers`, `_treasuryOwner` and `_swapContract`.
...
For troubleshooting, the sepolia deployment script (`scripts/dv-test.js`) now
prints all constructor arguments before deploying `INVTRON_DAO` so malformed or
zero addresses can be caught early.
...
A generic front-end is available in the `ui/` folder. Serve the contents of this directory with any static server. After running `scripts/dv-main.js` (mainnet) or `scripts/dv-test.js` (sepolia) the deployed contract address is written to `ui/deployInfo.json` and `ui/app.js` reads the address from this file automatically. The interface loads the contract ABI from `ui/INVTRON_DAO.json` and automatically generates a form for every read and write function. Both deployment scripts load constructor parameters from `constructor.env` and validate each address (`_priceFeedAddress`, `_initialCeo`, `_initialEndorsers`, `_treasuryOwner` and `_swapContract`) before broadcasting.
...
* **Reward Formula:** The token holder receives 90% of the voting power recorded when they cast their vote, with voting power defined as 0.5% of the USD value of their token balance (plus any delegated balance) capped at 10% of the funding request. Rewards are calculated as `votingPowerAtVote / 10` in USD(6) and converted to INV using the current oracle price. Delegated votes send the remaining 10% to the delegatee, who also retains the full reward from their own voting power.
